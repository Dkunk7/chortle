...
const targetWord = validAnswers[Math.floor(dayOffset)]
...
function stopInteraction() {
    document.removeEventListener("click", handleMouseClick) // REMOVE event listener 
...
function handleKeyPress(e) {
    ...
    if (e.key.match(/^[a-z]$/)) { // If the key pressed is a letter from a-z, "press key" with that key to fill the tile
        pressKey(e.key)
        return
    }
}
...
function pressKey(key) {
    ...
    if (activeTiles.length >= wordLength) return // If the active tiles is greater than/euqal to the guess limit, stop user from typing more
    const nextEmptyTile = guessGrid.querySelector(":not([data-letter])") // Finds the first tile that doesn't already have a letter in it
...
function deleteKey() {
    ...
    delete lastTile.dataset.state // I think these lines might be two prime examples of why data attributes are good to use?
    delete lastTile.dataset.letter
}

function submitGuess() {
    const activeTiles = [...getActiveTiles()] // Converts results to an array
...
function showAlert(message, duration = 1000) {
    ...
    alertContainer.prepend(alert) // Prepend adds to the top of the list, so the newest alert will appear above the others
    if (duration == null) return

    setTimeout(() => {
        alert.classList.add("hide") // Adds hide class to the alert, make it fade after 1 second
        alert.addEventListener("transitionend", () => { // Once the fade transition is over, the alert is removed
            alert.remove()
        })
    }, duration)
}
